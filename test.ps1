...
function Check-TableChanges {
    ...
    try {
        ...
        if ($TableName -eq "Appointment") {
            $query = "SELECT TOP 4000 [$ColumnName], AppDateTime, Pharmacist, AddUser, CustomerPHN, CustomerName, CustomerAddress, CustomerPhone, AppNote, AppCreatedDateTime FROM [$TableName] ORDER BY [$ColumnName] DESC"
        }
        elseif ($TableName -eq "Communications") {
            $query = "SELECT TOP 4000 [$ColumnName], PHN, Type, EventDateTime, Reference, Resource FROM [$TableName] ORDER BY [$ColumnName] DESC"
        }
        elseif ($TableName -eq "DRUG") {
            $query = "SELECT TOP 4000 [$ColumnName], DGDESC, DGUM, DGTYPE, DGQ1, DGQ2, DGQ3, DGC1, DGC2, DGC3, DGSHELF, DGINVNO, DGGEN1, DGGEN2, DGDATE, DGHAS1, DGHAS2, DGHAS3, DGSUPPLIER, DGLINE1, DGLINE2, DGLINE3, DGORDER1, DGORDER2, DGORDER3, DGMKUP, DGINVNO2, DGINVNO3, DGTRADE, DGSIG, DGEXPIRE, DG2OLD, DGWARN, DGU1, DGU2, DGU3, DGU4, DGU5, DGU6, DGU7, DGU8, DGU9, DGU10, DGU11, DGU12, DGUMON, DGPACK, DGSIGCODE, DGCOUNSEL, DGMSP, DGDAYRATE, DGWKRATE, DGMONRATE, DGPST, DGGST, DGGRACE, DGLCADIN, DGLCACOST, DGPACMED, DGPRICE, DGTXMKUP, DGBIN, DGUSED, DGUSE, DGUPC, DGCATEGORY, AddDatetime FROM [$TableName] ORDER BY [$ColumnName] DESC"
        }
        elseif ($TableName -eq "CALLBACK") {
            $query = "SELECT TOP 4000 [$ColumnName], CBDATE, CBSURNAME, CBGIVEN, CBDRUG, CBNOTE, CBPHONE, CBRXNUM, CBPREV, CBUSER, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime, CBCOMID FROM [$TableName] ORDER BY [$ColumnName] DESC"
        }
        elseif ($TableName -eq "CHANGES") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], CGNUM, CGLIM, CGQTY, CGBR, CGDEA, CGDRLAST, CGDR1ST, CGDATE, CGUSER, CGSIG, CGMSG, CGDRCOLL, CGACT, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime, CGDRUG, CGDIN FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
        ...
        elseif ($TableName -eq "CHGDRUG") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], DGDIN, DGDESC, DGDATE, DGUSER, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime, ORDERSTATUS FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
                # Skip large table in optimized mode to avoid false positives
                $query = $null
            }
        ...
        elseif ($TableName -eq "DOCUMENTS") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], DTNUM, DTDATE, DTTYPE, DTNAME, DTBATCHNUM, DTSOURCE, DTSOURCEFILE, DTFLAG, DTAILMENT, DTNOTE FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
                # Skip large table in optimized mode to avoid false positives
                $query = $null
            }
        ...
        elseif ($TableName -eq "ERX") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], XNUM, XBR, XSUPPLY, XPERIOD, XREFILLS, XQTY, XDRUG, XSIG, XDIN, XDEA, XDRSURNAME, XDRGIVEN, XCOLL, XDATE, XSTATUS, XPHN, XSURNAME, XGIVEN, XSTREET1, XSTREET2, XCITY, XPROV, XPC, XCTRY, XNOSUB, XRETADDR, XSEX, XBIRTH, XEXTERNALID, XNOTE, XHOME, XRB, XAREA, XPHONE, XENDDATE, XPRN, XSTAT, XORIGRX, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime, XPKG, XADAPT, XFREQ, XCREATED, XORIGERX, XMSG1, XMSG2, XMSG3, XALLERGY, XSITE, XROUTE, XADMIN, XCELL, XEMAIL, XPLANS, XPRICE, XADMNO, XUSERNOTE, XAIG, XSOURCE, XFAX, XRECORDINUSE, XPRIORITY, XDRFISRTTXID, XDRFISRTMEDID, XCONDITION, XAssignToERxUser, XPickUp FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
        ...
        elseif ($TableName -eq "PACMED") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], Patient, PHN, Home, HomeAddr, RB, DIN, Drug, MedDate, MedTime, Qty, Dr, Rx, Userid, SIG, RefLeft, PacType, Warning, DaysSupply, SentFlag, Dispenser, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime, DRID, ORIGDATE, PREVDATE, COST, PHARMID FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
                # Skip large table in optimized mode to avoid false positives
                $query = $null
            }
        ...
        elseif ($TableName -eq "PRESETMESSAGES") {
            $query = "SELECT TOP 4000 [$ColumnName], MSGDESC, MSGTEXT FROM [$TableName] ORDER BY [$ColumnName] DESC"
        }
        elseif ($TableName -eq "REFILL") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], RERXNUM, RERXBR, REEFDATE, RECOST, REFEE, RECOPAY, REQTY, REUSERID, REISORIG, REREVDATE, REXREF, REREVUSER, REPLAN, REAUTH, REUPCHG, RECMPDCHG, REPAY, REJUDGE, REREASON, REAUXBILL, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime, RETRACE, READAPT, REBADGE, REREFERENCE, RERESPONSE, REDIN, REPINID, REPINEVENTID, SIGNINGS, RECENTRAL, REBCSA FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
                # Skip large table in optimized mode to avoid false positives
                $query = $null
            }
        ...
        elseif ($TableName -eq "SCANS") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], SCBAR, SCFILE, SCDATE, SCBATCHNUM, SCTYPE, SCSOURCE, SCSOURCEFILE, SCFLAG, SCORDER FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
                # Skip large table in optimized mode to avoid false positives
                $query = $null
            }
        ...
        elseif ($TableName -eq "TXNS") {
            if ($FullScan) {
                $query = "SELECT TOP 4000 [$ColumnName], PLANID, ADJDATE, RX, AMT, CARID, SOURCE, AddDatetime, AddUsername, UpdateUsername, UpdateDatetime FROM [$TableName] ORDER BY [$ColumnName] DESC"
            } else {
                # Skip large table in optimized mode to avoid false positives
                $query = $null
            }
        ...
        elseif ($TableName -eq "WINMAIL_RECEIVED") {
            $query = "SELECT TOP 4000 [$ColumnName], EntryDate, Message, Guid, NAME, FAXNUM, PAGES, FILENAME, Note, SRFaxID, SRFaxFileName, Unread FROM [$TableName] ORDER BY [$ColumnName] DESC"
        }
        else {
        ...
        while ($reader.Read()) {
            ...
            elseif ($TableName -eq "ERX") {
                ...
                $addDatetime = if ($reader.IsDBNull(39)) { "" } else { $reader.GetValue(39).ToString() }
                $addUsername = if ($reader.IsDBNull(40)) { "" } else { $reader.GetValue(40).ToString() }
                $updateUsername = if ($reader.IsDBNull(41)) { "" } else { $reader.GetValue(41).ToString() }
                $updateDatetime = if ($reader.IsDBNull(42)) { "" } else { $reader.GetValue(42).ToString() }
                $xpkg = if ($reader.IsDBNull(43)) { "" } else { $reader.GetValue(43).ToString() }
                $xadapt = if ($reader.IsDBNull(44)) { "" } else { $reader.GetValue(44).ToString() }
                $xfreq = if ($reader.IsDBNull(45)) { "" } else { $reader.GetValue(45).ToString() }
                $xcreated = if ($reader.IsDBNull(46)) { "" } else { $reader.GetValue(46).ToString() }
                $xorigerx = if ($reader.IsDBNull(47)) { "" } else { $reader.GetValue(47).ToString() }
                $xmsg1 = if ($reader.IsDBNull(48)) { "" } else { $reader.GetValue(48).ToString() }
                $xmsg2 = if ($reader.IsDBNull(49)) { "" } else { $reader.GetValue(49).ToString() }
                $xmsg3 = if ($reader.IsDBNull(50)) { "" } else { $reader.GetValue(50).ToString() }
                $xallergy = if ($reader.IsDBNull(51)) { "" } else { $reader.GetValue(51).ToString() }
                $xsite = if ($reader.IsDBNull(52)) { "" } else { $reader.GetValue(52).ToString() }
                $xroute = if ($reader.IsDBNull(53)) { "" } else { $reader.GetValue(53).ToString() }
                $xadmin = if ($reader.IsDBNull(54)) { "" } else { $reader.GetValue(54).ToString() }
                $xcell = if ($reader.IsDBNull(55)) { "" } else { $reader.GetValue(55).ToString() }
                $xemail = if ($reader.IsDBNull(56)) { "" } else { $reader.GetValue(56).ToString() }
                $xplans = if ($reader.IsDBNull(57)) { "" } else { $reader.GetValue(57).ToString() }
                $xprice = if ($reader.IsDBNull(58)) { "" } else { $reader.GetValue(58).ToString() }
                $xadmno = if ($reader.IsDBNull(59)) { "" } else { $reader.GetValue(59).ToString() }
                $xusernote = if ($reader.IsDBNull(60)) { "" } else { $reader.GetValue(60).ToString() }
                $xaig = if ($reader.IsDBNull(61)) { "" } else { $reader.GetValue(61).ToString() }
                $xsource = if ($reader.IsDBNull(62)) { "" } else { $reader.GetValue(62).ToString() }
                $xfax = if ($reader.IsDBNull(63)) { "" } else { $reader.GetValue(63).ToString() }
                $xrecordinuse = if ($reader.IsDBNull(64)) { "" } else { $reader.GetValue(64).ToString() }
                $xpriority = if ($reader.IsDBNull(65)) { "" } else { $reader.GetValue(65).ToString() }
                $xdrfisrttxid = if ($reader.IsDBNull(66)) { "" } else { $reader.GetValue(66).ToString() }
                $xdrfisrtmedid = if ($reader.IsDBNull(67)) { "" } else { $reader.GetValue(67).ToString() }
                $xcondition = if ($reader.IsDBNull(68)) { "" } else { $reader.GetValue(68).ToString() }
                $xassigntoerxuser = if ($reader.IsDBNull(69)) { "" } else { $reader.GetValue(69).ToString() }
                $xpickup = if ($reader.IsDBNull(70)) { "" } else { $reader.GetValue(70).ToString() }
...
try {
    ...
    if ($FullScan) {
        ...
        Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Large tables (CHGDRUG, DOCUMENTS, CHANGES, PACMED, PATIENT, REFILL, RX, SCANS, TXNS) skipped to prevent false positives" -ForegroundColor Gray
    }
